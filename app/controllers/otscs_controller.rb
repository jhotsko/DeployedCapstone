class OtscsController < ApplicationController
  def create
    ...
    if @otsc.save
      ...
      if num_otscs == 1
        CadetMailer.with(cadet: @cadet, otsc: @otsc).otsc_one_email.deliver_later
        CadetMailer.with(cadet: @cadet, admin: @sqcc, otsc: @otsc).otsc_admin_email.deliver_later
      elsif num_otscs == 2
        CadetMailer.with(cadet: @cadet, otsc: @otsc).otsc_two_email.deliver_later
        CadetMailer.with(cadet: @cadet, admin: @sqcc, otsc: @otsc).otsc_admin_email.deliver_later
        @task = Task.create!(:date_created => @otsc.date, :date_due => @otsc.date.next_day(7), :description => "Memo for second OTSC", :completed => 0, :cadet_id => @cadet.id)
